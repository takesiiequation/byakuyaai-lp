...
// 「できれば」の推奨として案内する。
//
// 「カメラの動きは5種類」セクション(2026-07-25追加)の根拠:
...
// 「カメラの動きは5種類」の実演カード。動画は全て 9:16(720x1280)・約5秒・
...
const CAMERA_MOTIONS: ReadonlyArray<{
  ...
  },
  {
    ...
    how: "カメラの向きは変えず、体ごと真横に半歩〜1歩ずれた位置で2枚目を撮ります。正面から見せたい設備に向いています。",
    where: "洗面・キッチンなど、正面から見せたい設備まわり",
  },
  {
    name: "その場で見わたす(パン)",
    diagram: "pan",
    ...
    how: "足は動かさず、その場に立ったまま体の向きだけ変えて2枚目を撮ります(90°以内)。広がりが伝わります。",
    where: "バルコニー・テラス・眺望・浴室・洋室(寝室)など下がれない部屋",
  },
  {
    name: "回り込む(アーク)",
    diagram: "arc",
    ...
    how: "お部屋の隣り合う2つの角から、どちらもお部屋の中央を向いて撮ります。真ん中に見せたいものがあるお部屋で効果的です。",
    where: "広いリビング・ダイニング(中央にソファやアイランドキッチンがある部屋)",
  },
  {
    name: "見上げる",
    diagram: "tilt",
    ...
    <Shell maxWidthClassName="max-w-lg lg:max-w-4xl xl:max-w-6xl">
      ...
      <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5 sm:p-8">
        ...
        <div className={`${NARROW_BLOCK} mt-[1.2em]`}>
          ...
          <p>
            各お部屋の写真は、できるだけ<strong>2枚1組</strong>でお送りください。同じ部屋を、少しだけ位置や向きを変えた2つの場所から撮っていただくだけで、その2枚から自然なカメラ移動のある映像を作ることができます。1枚目が映像の「始まり」、2枚目が「終わり」になり、その間をなめらかにつないだ映像に仕上がります。
          </p>
          <p>
        ...
        <div className={`${NARROW_BLOCK} mt-[1.2em]`}>
          <h2>カメラの動きは5種類。2枚の撮り方で決まります</h2>
          <p>
            上の作例は、隣り合う角から中央を向いて撮った「回り込む(アーク)」の動きです。カメラの動きはこれだけではありません。<strong>2枚をどう撮ったかで、映像の動き方が決まります</strong>。同じ向きのまま前に進んで撮れば前へ進む映像に、その場で向きだけ変えて撮れば見わたす映像になります。
          </p>
          <p>
            どの動きが向いているかは、お部屋の形で決まります。たとえば洗面所のような狭い場所で前へ進もうとすると、すぐ壁にぶつかってしまい2枚目が撮れません。こうした場所は、カメラの向きを変えずに体ごと真横にずれる「真横にスライド」が向いています。逆に、バルコニーのように広がりを見せたい場所は、その場で向きだけ変える「パン」が向いています。下の5つの実演を参考に、お部屋ごとに撮りやすい動きをお選びください。
          </p>
        </div>

        {/* 5種のカメラ移動の実演カード。非プローズ要素なので広い幅を使う。
            縦積み(モバイル)→2列(sm)→3列(lg)→5列(xl・横一列)。 */}
        <div className="mt-[1.2em] mx-auto max-w-prose lg:max-w-3xl xl:max-w-6xl">
          ...
          <table>
            ...
            <tbody>
              ...
              <tr>
                <td>洗面・キッチン</td>
                <td>
                  真横にスライド:カメラの向きは変えず、体ごと真横に半歩〜1歩ずれて2枚
                </td>
              </tr>
              <tr>
                <td>バルコニー・眺望</td>
                <td>
                  その場で見わたす(パン):足は動かさず、景色を追うように向きだけ変えて2枚(90°以内)
                </td>
              </tr>
              <tr>
                <td>浴室・洋室(寝室)</td>
                <td>
                  その場で見わたす(パン):下がるスペースがないので、その場で向きだけ変えて2枚
                </td>
              </tr>
              <tr>
                <td>広いリビング・ダイニング</td>
                <td>
                  回り込む(アーク):隣り合う2つの角から、どちらも中央を向いて2枚
                </td>
              </tr>
              <tr>
                <td>トイレ</td>
        ...
        <div className={`${WIDE_BLOCK} mt-[1.2em]`}>
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
            ...
            <p className="mt-1 text-sm text-amber-700">
              2枚の間をつなぐ映像が作れず、不自然な仕上がりになってしまいます。1枚目を撮った場所を起点に、上の5つの動き(前へ進む・真横にスライド・その場で見わたす・回り込む・見上げる)のいずれかで2枚目を撮ってください。
            </p>
          </div>
        </div>
        ...
        <div className={`${NARROW_BLOCK} mt-[1.2em]`}>
          ...
          <ul>
            <li>
              リビング(2枚1組):お部屋いちばんの魅力なので、できれば2組(4枚)あると動画がいきいきします
            </li>
            <li>キッチン(2枚1組)</li>
            <li>浴室(2枚1組)</li>
            <li>洗面(2枚1組)</li>
            <li>玄関または廊下(2枚1組)</li>
            <li>外観(2枚1組・建物全体がわかるもの。動画の締めくくりに使われる大切なカットです)</li>
            <li>バルコニーからの眺望(2枚1組・あれば)</li>
            <li>マイソク(図面):文字が読める鮮明さで撮影・スキャンをお願いします</li>
          </ul>
          <p>
            1つのお部屋につき2枚1組で、6〜10部屋分(合計12〜20枚)が目安です。1組が動画の1カットになりますので、同じ部屋の似た構図を何組も送っていただくより、部屋の種類を増やしていただいたほうが、動画の中でバラエティ豊かに活きます。
          </p>
