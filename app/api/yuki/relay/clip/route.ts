// デスクユキ 映像の作り直しリレー(2026-09-06): コンテナからだけ叩かれる内部API(認証=ジョブの短命トークン)
//   POST {client_id, job_id, token, approval_id, scene, image_url, prompt, duration?, resolution?, aspect_ratio?, dry_run?} → {ok, request_id, credits}
//   GET  ?client_id&job_id&token&request_id → {ok, status:"pending"|"done", video_url?, credits?}
import { NextRequest, NextResponse } from "next/server";
import { verifyClipToken, submitClip, pollClip } from "@/app/_lib/yuki_clips";
...
export async function POST(req: NextRequest) {
  ...
  const v = await verifyClipToken(clientId, jobId, token);
