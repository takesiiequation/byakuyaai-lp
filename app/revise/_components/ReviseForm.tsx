...
export default function ReviseForm({
  ...
  const canSubmit = changed.length > 0 && !hasEmptyChange && !submitting;
  ...
  async function handleConfirmSend() {
    ...
    try {
      const res = await fetch("/api/revise/submit", {
        ...
        body: JSON.stringify({ approvalId, edits: changed }),
      });
    ...
    <div className="space-y-5">
      ...
      {caption && (
        <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/5 sm:p-5">
          ...
          <p className="mt-2 whitespace-pre-wrap text-sm text-[var(--brand-ink)]">
            {caption}
          </p>
          <p className="mt-2 text-[11px] text-[var(--brand-gray-light)]">
            ※キャプションの修正はご担当までご連絡ください
          </p>
        </div>
      )}
              ...
              {changed.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-0.5 text-[var(--brand-ink)]">
                      {c.new_text}
                    </div>
                  </li>
                );
